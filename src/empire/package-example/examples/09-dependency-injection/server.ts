/**
 * 09 - Dependency Injection (package-example mirror)
 *
 * Same as Empire's examples/09-dependency-injection, but importing from
 * the published "empire-ts" package instead of the framework's source
 * tree. Demonstrates Empire's DI container wired into a real app via
 ...
 *   POST /records all resolve the *same* repository instance.
 ...
 *   /records endpoint.
 *
 * Run: npx tsx examples/09-dependency-injection/server.ts   (from package-example/)
 * Open: http://localhost:9009
 *
 * Try it:
 *   curl http://localhost:9009/records                                # seeded records
 *   curl http://localhost:9009/records/1                               # one record
 *   curl -X POST http://localhost:9009/records \
 *     -H "Content-Type: application/json" \
 *     -d '{"name":"Third sample","value":30}'                          # 201, new record
 *   curl http://localhost:9009/records/nope                            # 404
 *   curl http://localhost:9009/upstream-summary                        # count + total value,
 *                                                                       # fetched via a real
 *                                                                       # HTTP round trip
 */
...
// --- HTTP client + upstream API service ---------------------------------
...
// honest instead of a bare non-null assertion.
