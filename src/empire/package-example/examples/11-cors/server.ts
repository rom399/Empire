/**
 * 11 - CORS (package-example mirror)
 *
 * Same as Empire's examples/11-cors, but importing from the published
 * "empire-ts" package instead of the framework's source tree.
 * Demonstrates createCorsMiddleware(), wrapping requests the same way
 * createLoggerMiddleware(logger) does - no changes to Router or Context
 * needed to add this.
 *
 ...
 *   origin, no credentials) via the multi-policy form
 *
 * Run: npx tsx examples/11-cors/server.ts   (from package-example/)
 * Open: http://localhost:9011
 *
 * Try it (curl doesn't enforce CORS itself, but shows the exact headers
 * a browser would act on):
 *   curl -i http://localhost:9011/api/data \
