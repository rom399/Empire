/**
 * 07 - Request Body Size Limit (package-example mirror)
 *
 * Same as Empire's examples/07-body-size-limit, but importing from the
 ...
 *
 * Run: npx tsx examples/07-body-size-limit/server.ts   (from package-example/)
 * Open: http://localhost:9007
 *
 * Try it:
 *   curl -X POST http://localhost:9007/echo -d "short body"                     # 200
 *   node -e "process.stdout.write('x'.repeat(2000))" | \
 *     curl -X POST --data-binary @- http://localhost:9007/echo                  # 413
 */
