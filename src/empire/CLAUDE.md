...
- **Dependencies**: no runtime dependencies, and no validation library even as a dev dependency — `validate()` accepts any Standard Schema validator that the *user* installs (see `doc/features/REMOVE_ZOD.md`). Do not add npm packages without being asked.
