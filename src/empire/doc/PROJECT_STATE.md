...
Phase 9.2 (Core Class Test Coverage) is now fully complete - every item
in its `PLAN.md` checklist is checked off, including `HttpError.test.ts`,
`BadRequestError.test.ts`, and `MimeTypes.test.ts`, which turned out to
already be covered rather than genuinely missing.
